...
async function get(
  ...
  const response = await fetch(
    origin + "/api/portal?" + new URLSearchParams({ view, ...params }),
    {
      headers: { "x-study-portal": portal, cookie: cookieHeader(cookies) },
      signal: AbortSignal.timeout(30_000),
    },
  );
...
async function main() {
  assert.ok(
    ...
    "Smoke must use a separate unprivileged port",
  );
  ...
  assert.ok(
    ...
    "Smoke is restricted to a local test database",
  );
  ...
  const env: NodeJS.ProcessEnv = {
    ...
    NODE_ENV: buildDirectory ? "production" : "development",
    APP_HOST: "127.0.0.1",
    APP_ORIGIN: origin,
    PORT: String(port),
    ...
    NEXT_DIST_DIR: buildDirectory ?? ".cache/portals-test",
  };
  ...
  const researcher = (
    await command("research", "bootstrap", {
      username: prefix + "_r",
      password,
      pseudonym: prefix + "_researcher",
    })
  ).actor;
  ...
  await command(
    "research",
    "bootstrap",
    { username: prefix + "_r2", password, pseudonym: prefix + "_another" },
    409,
  );
  ...
  const preview = await command("target", "inspectInvitation", {
  ...
  const target = (
    await command("target", "register", {
      token: targetInvite.token,
      username: prefix + "_t",
      password,
      pseudonym: prefix + "_target",
      consent,
    })
  ).actor;
  ...
  await command(
    ...
    {
      token: targetInvite.token,
      username: prefix + "_again",
      password,
      pseudonym: prefix + "_again",
      consent,
    },
    404,
  );
  ...
  const friend = (
    await command("friend", "register", {
      token: friendInvite.token,
      username: prefix + "_f",
      password,
      pseudonym: prefix + "_friend",
      consent,
    })
  ).actor;
  ...
  const first = await command("target", "send", {
    roomId,
    text: "准备聊天：这是本人发送的消息。",
    idempotencyKey: firstKey,
  });
  const repeated = await command("target", "send", {
    roomId,
    text: "准备聊天：这是本人发送的消息。",
    idempotencyKey: firstKey,
  });
  ...
  assert.deepEqual(
    targetRoom.messages.map((m: Data) => m.mine),
    [true, false],
  );
  assert.deepEqual(
    friendRoom.messages.map((m: Data) => m.mine),
    [false, true],
  );
  ...
  checked(
    "bidirectional ordered human chat, idempotency, no AI or formal study records",
  );
  ...
  await command(
    ...
    "register",
    {
      token: friend2Invite.token,
      username: prefix + "_f2",
      password,
      pseudonym: prefix + "_friend2",
      consent,
    },
    200,
    friend2Jar,
  ...
  await command(
    "friend",
    "send",
    { roomId, text: "不应进入别人的聊天", idempotencyKey: randomUUID() },
    404,
    friend2Jar,
  );
  ...
  const legacy = await fetch(origin + "/api/study?view=export", {
    headers: { "x-study-portal": "target", cookie: cookieHeader() },
  });
  assert.equal(legacy.status, 403);
  checked(
    "another friend and research staff cannot read room text; real participants cannot enter legacy study",
  );
  ...
  await command(
    "friend",
    "send",
    { roomId, text: "暂停后不应发送", idempotencyKey: randomUUID() },
    409,
  );
  ...
  const withdrawal = await command("friend", "withdraw", {
    destroyContent: true,
  });
  ...
  await command(
    "friend",
    "send",
    { roomId, text: "撤回后不应重开旧房间", idempotencyKey: randomUUID() },
    409,
  );
  checked(
    "withdrawal clears one portal login, destroys text and cannot reopen the old room",
  );

  stage = "public schema isolation";
  const publicRows = await admin.$queryRawUnsafe<Array<{ count: bigint }>>(
    'SELECT count(*) AS count FROM public."Participant" WHERE "pseudonym" LIKE $1',
    prefix + "%",
  );
  ...
  assert.equal(await db.generation.count(), 0);
  checked(
    "all four test accounts stayed in the disposable schema; public contains none",
  );
...
main()
  ...
  .finally(async () => {
    try {
      ...
      console.error(
        "Portal smoke cleanup failed; inspect disposable portal_http_ schema.",
      );
      process.exitCode = 1;
    }
  });
