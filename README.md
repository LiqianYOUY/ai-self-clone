...
网页入口由 GitHub Pages 展示，完整实验部署在独立树莓派服务，参与者直接用浏览器打开，无需下载项目或运行终端。Mac 与树莓派均通过 187 项回归和生产构建。Tailscale 公网 HTTPS 与游戏页面已验证可访问；此前部分公共 DNS 查询结果有差异，可能影响个别网络。Cloudflare 备用未启用。树莓派需要保持开机联网，详见 [验证记录](guide/VERIFICATION.md)。
...
本人风格提炼及稳定性更新已于 2026-09-17 部署到树莓派：保存时识别本人发言、提取表达习惯，回复时按当前场景选择相关原话，区分人物事实与表达参考；主持人可试聊并手动纠正，保存后用于新局。设备现用本机 Qwen3.5 4B，采用温度 0 和启动预热，已完成本人局与真实 AI 局各五轮验收。合成样本的重复性有改善，仍偶有无依据的近况或推断；真实发起者相似度尚未验证，详见[验证记录](guide/VERIFICATION.md)。这是语料与上下文层的学习，不是基础模型权重微调。
