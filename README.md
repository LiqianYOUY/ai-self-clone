...
当前源码新增本人风格提炼：保存时识别本人发言、提取表达习惯，回复时选择相关原话示例；主持人可试聊并手动纠正，保存后用于新局。Mac 已通过 150 项回归、生产构建和真实 4B 合成场景检查；树莓派仍需更新部署后才能使用，详见[验证记录](guide/VERIFICATION.md)。这是语料与上下文层的学习，不是基础模型权重微调。
